...
// 宠物管理
angular.module('fiona').controller('MsgsettingController', function($scope, $http, commons) {
    ...
    $scope.selectAll= function () {
        angular.forEach($scope.msgsetting, function (curemanager) {
    ...
    $scope.search = function () {
        ...
        // 分页查询  /business/api/v2/msgsetting
        // $http.get('/server/api/v2/msgsetting/page.json', {'pageSize':$scope.pagination.pageSize, 'pageNumber':$scope.pagination.pageNumber, 'filters':$scope.filters}).success( function ( data, status, headers, config ) {
        ...
        $http.post('/server/api/v2/msgsetting/page.json', {'pageSize':$scope.pagination.pageSize, 'pageNumber':$scope.pagination.pageNumber, 'filters':$scope.filters}).success( function ( data, status, headers, config ) {
            console.log( data );

            $scope.msgsetting = data.data.content;

            // $http.post('/business/api/v2/msgsetting', data).success( function ( data, status, headers, config ) {
            //     console.log( data );
            //     alert("Submit OK");
            ...
            // angular.forEach($scope.msgsetting, function (curemanager, i) {
    ...
    // Form 界面
    $scope.msgsettingubmit = function () {

        $scope.curemanagerform.submitted = true;

        alert("MsgsettingSex: " + $scope.curemanager.curemanagerSex.valueNameCn);
        ...
        if ($scope.curemanagerform.$valid) {
            ...
            $http.post('/business/api/v2/msgsetting', $scope.curemanager).success( function ( data, status, headers, config ) {
                console.log( data );
                alert("Submit OK");
            }).error(function(data,status,headers,config) { //     错误
                alert("Status: " + status);
    ...
    $scope.update = function (id) {
        ...
        {
            angular.forEach($scope.msgsetting, function(data,index,array){
                if(data.id== id)
                {
    ...
    $scope.remove= function (id) {
        ...
        $http.delete('/business/api/v2/msgsetting/' + id).success(function(data, index, array){
            commons.danger("删除成功");
        }).error(function (data) {
            commons.danger("删除失败");
        });
