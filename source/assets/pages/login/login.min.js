// angular.module('fiona', ['fiona.services']);
angular.module('fiona', ['ngRoute','fiona.services'])
    .controller('SignupController', function ($scope, $http, commons, Auth){

        $http.defaults.headers.post['Content-Type']= 'application/json';

        // alert("Invoke: SidebarController");
        $scope.submitted = false;
        $scope.signup = {"name": "",  "password":""}

        $scope.signupForm = function() {

            if ($scope.signup_form.$valid) {

                // Submit as normal
                $http.post(commons.getAccountHostname() + "/api/v2/auth/login", $scope.signup).success(function (data, status, headers, config ) {

                    $scope.signup_form.submitted = true;

                    if(status!= 200 || data.code !=0 ){
                        $scope.error = true;
                        $scope.message = data.message || "登录失败";
                    }
                    else {
                        // 保存登录凭证
                        sessionStorage.setItem("authorization", data.data)

                        commons.setAuthorization(data.data);
                        
                        $http.get(commons.getAccountHostname() + "/api/v2/auth", {headers: {'authorization': data.data}}).success(function (data, status, headers, config ) {

                            $scope.menus = data.data;

                            sessionStorage.setItem("userName", data.data.name);

                        }).error(function (data, status, headers, config) {
                            console.log('加载用户凭证失败');
                        });

                        Auth.authorization = data.data;

                        console.log("SessionStorage Auth : " + sessionStorage.getItem("authorization"));

                        window.location.href = "index.html";
                    }

                }).error(function (data, status, headers, config) {
                    $scope.message = "登录失败显示错误信息";
                });
            } else {
                $scope.message = "请输入用户名和密码.";

                $scope.signup_form.submitted = true;
            }
        }
    });

//
// // 读取目录树
// function  ( $scope, $http, $window, commons) {
//
// };
